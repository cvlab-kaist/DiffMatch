...
class ETHInterval(data.Dataset):
    ...
    def __init__(self, root, path_list, source_image_transform=None, target_image_transform=None, flow_transform=None,
                 co_transform=None, severity=None, corruption_number=None, *args, **kwargs):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        if self.severity > 0:
            ...
            source_image = corrupt(source_image, severity=self.severity, corruption_number=self.corruption_number)
            target_image = corrupt(target_image, severity=self.severity, corruption_number=self.corruption_number)
            if resize:
                source_image = cv2.resize(source_image, (W_orig, H_orig))
                target_image = cv2.resize(target_image, (W_orig, H_orig))
