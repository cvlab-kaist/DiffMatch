...
def HPatchesdataset(root, csv_path, image_transform=None, flow_transform=None, co_transform=None,
                    use_original_size=False, get_mapping=False, image_size=None,
                    severity=0, corruption_number=0, **kwargs):
    ...
    if image_size is not None:
        test_dataset = HPatchesDataset(root, csv_path, image_transform, flow_transform, co_transform,
                                       use_original_size=use_original_size, get_mapping=get_mapping, image_size=image_size,
                                        severity=severity, corruption_number=corruption_number)
...
class HPatchesDataset(Dataset):
    ...
    def __init__(self, root, path_list, image_transform, flow_transform, co_transform, use_original_size=False,
                ...
                severity=0, corruption_number=0):
    ...
    def __getitem__(self, idx):
        ...
        if self.severity > 0:
            resize = self.corruption_number in [5, 6, 7, 8, 9, 13]
            if resize:
                H_orig, W_orig = img1.shape[:2]
                img1 = cv2.resize(img1, (224, 224))
                img2 = cv2.resize(img2, (224, 224))
            img1 = corrupt(img1, severity=self.severity, corruption_number=self.corruption_number)
            img2 = corrupt(img2, severity=self.severity, corruption_number=self.corruption_number)
            if resize:
                img1 = cv2.resize(img1, (W_orig, H_orig))
                img2 = cv2.resize(img2, (W_orig, H_orig))
