...
from utils_flow.flow_and_mapping_operations import (
    convert_flow_to_mapping, convert_mapping_to_flow,
    unormalise_and_convert_mapping_to_flow)

from .feature_correlation_layer import (compute_global_correlation,
                                        featureL2Norm)
...
def correlation_to_flow_w_soft_argmax(correlation, output_shape, temperature=1.0, apply_softmax=True,
                                      stable_softmax=False, return_all=False):
    """
    ...
    flow_est = F.interpolate(flow, size=output_shape, mode='bilinear', align_corners=True)
    flow_est[:, 0] *= output_shape[1] / w
    flow_est[:, 1] *= output_shape[0] / h

    if return_all:
        return flow_est, convert_flow_to_mapping(flow_est)
    return flow_est


def correlation_to_flow_w_argmax(correlation, output_shape=None, return_mapping=False, do_softmax=False, return_all=False):
    """
    ...
    if output_shape is not None and (H != output_shape[0] or W != output_shape[1]):
        flow_est = F.interpolate(flow_est, output_shape, mode='bilinear', align_corners=True)
...
def estimate_epe_from_correlation(correlation, flow_gt, mask_gt, pck_thresh_1=1.0, pck_thresh_2=3.0, do_softmax=False):
    ...
    if mask_gt is not None:
        mask_gt = F.interpolate(mask_gt.unsqueeze(1).float(), (h_flow, w_flow), mode='bilinear',
                                align_corners=True).squeeze(1).byte()
...
def features_to_flow(c_source, c_target, output_shape=None, return_mapping=False, return_all=False):
    """
    ...
    # b, h_s*w_s, h_t, w_t
    return correlation_to_flow_w_argmax(
        correlation_volume, output_shape=output_shape, return_mapping=return_mapping, return_all=return_all)
