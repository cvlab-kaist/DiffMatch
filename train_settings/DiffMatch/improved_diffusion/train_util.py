...
from ..eval_utils import extract_raw_features, initialize_flow, local_Corr
...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while True:
            ...
            for i, data in zip(indices, self.data): 
                ...
                # get raw corr and init flow
                with torch.no_grad():
                    raw_corr, trg_feat, _ = extract_raw_features(self.pyramid, target, source, target_256, source_256, self.feature_size)
                init_flow = correlation_to_flow_w_argmax(
                    raw_corr.view(B, 1, self.feature_size, self.feature_size, self.feature_size, self.feature_size),
                    output_shape=(self.feature_size, self.feature_size),
                )  
               
                # get local corr and normalize init flow
                coords = initialize_flow(init_flow.shape[0], self.feature_size, self.feature_size, dist_util.dev())
                coords_warped = coords + init_flow

                local_corr = local_Corr(
                    raw_corr.view(B, 1, self.feature_size, self.feature_size, self.feature_size, self.feature_size).to(dist_util.dev()),
                    coords_warped.to(dist_util.dev()),
                    self.radius,
                )

                local_corr = F.interpolate(
                    local_corr.view(B, (2 * self.radius + 1) ** 2, self.feature_size, self.feature_size),
                    size=self.feature_size,
                    mode='bilinear',
                    align_corners=True,
                )

                init_flow /= self.feature_size
                
                _, _, H, W = batch_ori.shape 
                batch_ori[:,0,:,:] /= W
                batch_ori[:,1,:,:] /= H      
                batch = F.interpolate(batch_ori, size=(self.image_size, self.image_size), mode='bilinear', align_corners=True)                     

                cond = {'y': None, 'local_corr': local_corr, 'init_flow': init_flow, 'trg_feat': trg_feat}

                self.run_step(batch, batch_ori, mask, cond)
    ...
    def run_step(self, batch, batch_ori, mask, cond):
    ...
    def forward_backward(self, batch, batch_ori, mask, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            micro_ori = batch_ori[i : i + self.microbatch].to(dist_util.dev())
